...
set
  gender = case
    when account_type = 'au_pair' and gender is null and full_name ilike any(array['%anna%', '%sofia%', '%ana%', '%emma%', '%lisa%']) then 'female'
    when account_type = 'au_pair' and gender is null then 'male'
    else gender
  end,
  birth_date = case
    when account_type = 'au_pair' and birth_date is null then
      (
        make_date(
          extract(year from current_date)::int - 20,
