-- A rolling 24-hour count alone allowed two message emails to arrive close
-- together. Enforce a calmer cadence atomically: at least 12 hours between
-- reservations and at most one reservation per recipient-local calendar day.
...
begin
  ...
  if exists (
    ...
    where reservation.recipient_id = p_recipient_id
      ...
      and (
        reservation.reserved_at > v_now - interval '12 hours'
        or (reservation.reserved_at at time zone v_timezone)::date =
          v_recipient_today
      )
  ) then
    return null;
...
begin
  ...
  select max(reservation.reserved_at)
